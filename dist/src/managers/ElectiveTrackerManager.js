...
class ElectiveTrackerManager {
    ...
    `, [classId, classInfo.grade, requiredElectives, assignedCount, missingElectives, status]);
            ...
            if (status === 'incomplete') {
                ...
                await this.dbManager.runSQL(`INSERT INTO assignment_alerts (class_id, alert_type, severity, message, is_resolved)
         VALUES (?, 'missing_electives', ?, ?, 0)`, [classId, severity, `${classInfo.grade}/${classInfo.section} sınıfında ${missingElectives} seçmeli ders eksik`]).catch(() => { });
            }
            else if (status === 'over_assigned') {
                await this.dbManager.runSQL(`INSERT INTO assignment_alerts (class_id, alert_type, severity, message, is_resolved)
         VALUES (?, 'over_assignment', 'warning', ?, 0)`, [classId, `${classInfo.grade}/${classInfo.section} sınıfında fazla seçmeli ders ataması var`]).catch(() => { });
            }
            else {
                // Resolve alerts when complete
                await this.dbManager.runSQL(`UPDATE assignment_alerts SET is_resolved = 1, resolved_at = CURRENT_TIMESTAMP WHERE class_id = ? AND is_resolved = 0`, [classId]).catch(() => { });
            }
            return {
                classId: classId,
