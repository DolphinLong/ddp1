...
// Interface definitions for suggestion system
...
export class SuggestionEngine {
  ...
  async generateSuggestions(
    ...
    try {
      const defaultCriteria: SuggestionCriteria = {
        classId,
      ...
      const assignedLessonIds = assignedElectives.map((item: any) => item.lesson_id);

      // Filter out already assigned electives
      const unassignedElectives = availableElectives.filter(
        lesson => !assignedLessonIds.includes(lesson.lessonId)
      );
      ...
      const teachersById = new Map<number, { id: number; name: string; subject?: string }>();
      ...
      const workloadRows = await this.dbManager.getAll(`
        ...
        GROUP BY t.id
      `);
      const workloadPct = new Map<number, number>();
      ...
      for (const r of assignCountRows) assignmentCountMap.set(r.teacher_id, r.cnt || 0);
      ...
      for (const r of popularityRows) popularityMap.set(r.lesson_id, r.cnt || 0);
      ...
      // Prefetch schedules
      const classSlots = await this.dbManager.getAll(
        'SELECT day_of_week, time_slot FROM schedule_items WHERE class_id = ?', [classId]
      );
      const classSlotSet = new Set<string>(classSlots.map((s: any) => `${s.day_of_week}|${s.time_slot}`));

      // Candidate teachers per lesson: previously taught OR subject match
      // Previously taught
      const placeholders = lessonIds.map(() => '?').join(',');
      const taughtRows = await this.dbManager.getAll(
        `SELECT DISTINCT ta.lesson_id, t.id as teacher_id, t.name
         FROM teacher_assignments ta
         JOIN teachers t ON ta.teacher_id = t.id
         WHERE ta.lesson_id IN (${placeholders})`, lessonIds
      );
      const candidatesByLesson = new Map<number, Set<number>>();
      for (const id of lessonIds) candidatesByLesson.set(id, new Set<number>());
      ...
      const allCandidateTeacherIds = Array.from(new Set(
        Array.from(candidatesByLesson.values()).flatMap(s => Array.from(s))
      ));
      const teacherScheduleMap = new Map<number, Set<string>>();
      if (allCandidateTeacherIds.length > 0) {
        const tph = allCandidateTeacherIds.map(() => '?').join(',');
        const schedRows = await this.dbManager.getAll(
          `SELECT teacher_id, day_of_week, time_slot FROM schedule_items WHERE teacher_id IN (${tph})`,
          allCandidateTeacherIds
        );
        for (const r of schedRows) {
          const key = `${r.day_of_week}|${r.time_slot}`;
          if (!teacherScheduleMap.has(r.teacher_id)) teacherScheduleMap.set(r.teacher_id, new Set());
      ...
      for (const lesson of unassignedElectives) {
        ...
        if (candSet.size === 0) {
          ...
          for (const id of teachersById.keys()) candSet.add(id);
        }
        for (const teacherId of candSet) {
          ...
          if (finalCriteria.preferTeachersWithLowWorkload) {
            ...
            if (assignCnt >= 3) score -= Math.min(30, assignCnt * 5);
          }
          // Popularity
          ...
          if (finalCriteria.avoidScheduleConflicts) {
            const tset = teacherScheduleMap.get(teacherId) || new Set<string>();
            let conflict = false;
            for (const k of tset) { if (classSlotSet.has(k)) { conflict = true; break; } }
          ...
          if (tinfo?.subject) {
            ...
            const lname = lesson.lessonName.toLowerCase();
            if (subj && (subj.includes(lname) || lname.includes(subj))) score += 10;
          }
      ...
      const enriched = [] as SuggestionWithDetails[];
  ...
  async scoreSuggestion(
    ...
    try {
      ...
      if (criteria.preferTeachersWithLowWorkload) {
        ...
        const assignCountRes = await this.dbManager.getOne(
  ...
  async applySuggestion(suggestionId: number): Promise<boolean> {
    ...
    try {
      ...
      const suggestion = await this.dbManager.getOne(
        'SELECT * FROM elective_suggestions WHERE id = ?',
        [suggestionId]
      );
   ...
   */
  private async calculateTeacherWorkload(teacherId: number): Promise<number> {
    try {
   ...
   */
  private async checkScheduleConflicts(classId: number, teacherId: number): Promise<boolean> {
    try {
      // Get class schedule
   ...
   */
  private async getLessonPopularity(lessonId: number): Promise<number> {
    try {
   ...
   */
  private async getAvailableElectiveLessons(grade: number): Promise<Array<{lessonId: number, lessonName: string}>> {
   ...
   */
  private async getTeachersForLesson(lessonId: number): Promise<Array<{teacherId: number, teacherName: string}>> {
   ...
   */
  private async checkTeacherSubjectMatch(teacherId: number, lessonId: number): Promise<boolean> {
    try {
   ...
   */
  private async cacheSuggestions(suggestions: SuggestionWithDetails[]): Promise<void> {
   ...
   */
  async getCachedSuggestions(classId: number): Promise<SuggestionWithDetails[]> {
