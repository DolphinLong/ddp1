...
export class LessonManager {
  ...
  async getAll(): Promise<Lesson[]> {
    ...
    return this.dbManager.getAll(
      'SELECT * FROM lessons WHERE school_type = ? ORDER BY grade ASC, name ASC',
      [currentSchoolType]
    );
  }
  ...
  async create(lessonData: Omit<Lesson, 'id' | 'created_at' | 'updated_at'>): Promise<Lesson> {
    // Check if lesson already exists for this grade
    const existing = await this.dbManager.getOne(
      'SELECT id FROM lessons WHERE name = ? AND grade = ?',
      [lessonData.name, lessonData.grade || 9] // Default to grade 9 if not provided
    );
    ...
    const result = await this.dbManager.runSQL(
      ...
      [lessonData.name, lessonData.grade || 9, lessonData.weekly_hours || 2, lessonData.is_mandatory, lessonData.school_type || 'İlkokul']
    );
  ...
  async update(id: number, lessonData: Partial<Lesson>): Promise<Lesson> {
    ...
    if (lessonData.name !== undefined && lessonData.grade !== undefined) {
      const existing = await this.dbManager.getOne(
        'SELECT id FROM lessons WHERE name = ? AND grade = ? AND id != ?',
        [lessonData.name, lessonData.grade, id]
      );
  ...
  async delete(id: number): Promise<boolean> {
    ...
    const scheduleCount = await this.dbManager.getOne(
      'SELECT COUNT(*) as count FROM schedule_items WHERE lesson_id = ?',
      [id]
    );
  ...
  async getByGrade(grade: number): Promise<Lesson[]> {
    const currentSchoolType = this.dbManager.getCurrentSchoolType();
    return this.dbManager.getAll(
      'SELECT * FROM lessons WHERE grade = ? AND school_type = ? ORDER BY name ASC',
      [grade, currentSchoolType]
    );
  }

  async getBySchoolType(schoolType: string): Promise<Lesson[]> {
    return this.dbManager.getAll(
      'SELECT * FROM lessons WHERE school_type = ? ORDER BY grade ASC, name ASC',
      [schoolType]
    );
  ...
  async getMandatoryLessons(grade: number): Promise<Lesson[]> {
    return this.dbManager.getAll(
      'SELECT * FROM lessons WHERE grade = ? AND is_mandatory = 1 ORDER BY name ASC',
      [grade]
    );
  }

  async getElectiveLessons(grade: number): Promise<Lesson[]> {
    return this.dbManager.getAll(
      'SELECT * FROM lessons WHERE grade = ? AND is_mandatory = 0 ORDER BY name ASC',
      [grade]
    );
  }

  async getTotalWeeklyHours(grade: number): Promise<number> {
    const result = await this.dbManager.getOne(
      'SELECT SUM(weekly_hours) as total FROM lessons WHERE grade = ?',
      [grade]
    );
    return result?.total || 0;
  }

  async getLessonStatistics(): Promise<any> {
  ...
  async searchLessons(query: string): Promise<Lesson[]> {
    return this.dbManager.getAll(
      'SELECT * FROM lessons WHERE name LIKE ? ORDER BY grade ASC, name ASC',
      [`%${query}%`]
    );
  }

  async getLessonsWithoutTeachers(): Promise<Lesson[]> {
    return this.dbManager.getAll(`
  ...
  async validateMEBCompliance(grade: number, schoolType: string = 'İlkokul'): Promise<{ isCompliant: boolean; issues: string[] }> {
    const issues: string[] = [];
    ...
    const requiredHours: { [key: string]: { [key: number]: number } } = {
  ...
  async createMEBCurriculum(grade: number, schoolType: string = 'İlkokul'): Promise<void> {
    // Clear existing lessons for this grade and school type
    await this.dbManager.runSQL(
      'DELETE FROM lessons WHERE grade = ? AND school_type = ?',
      [grade, schoolType]
    );

    // MEB curriculum data for different school types and grades
    const mebCurriculum: { [key: string]: { [key: number]: any[] } } = {
      'İlkokul': {
        1: [
    ...
    for (const lesson of curriculum) {
      await this.dbManager.runSQL(
        'INSERT INTO lessons (name, grade, weekly_hours, is_mandatory, school_type) VALUES (?, ?, ?, ?, ?)',
        [lesson.name, grade, lesson.weekly_hours, lesson.is_mandatory, schoolType]
      );
    }
  }
  ...
  async getGrades(): Promise<number[]> {
    const results = await this.dbManager.getAll(
      'SELECT DISTINCT grade FROM lessons ORDER BY grade ASC'
    );
  ...
  async getSchoolTypes(): Promise<string[]> {
    const results = await this.dbManager.getAll(
      'SELECT DISTINCT school_type FROM lessons ORDER BY school_type ASC'
    );
    return results.map(r => r.school_type);
  }

  async duplicateGradeCurriculum(fromGrade: number, toGrade: number, schoolType: string): Promise<void> {
   ...
   */
  async getAvailableElectiveLessons(grade: number): Promise<Lesson[]> {
    const currentSchoolType = this.dbManager.getCurrentSchoolType();
    return this.dbManager.getAll(`
      SELECT DISTINCT l.* 
      FROM lessons l
   ...
   */
  async getAssignedElectiveLessons(grade: number): Promise<any[]> {
    const currentSchoolType = this.dbManager.getCurrentSchoolType();
    return this.dbManager.getAll(`
      SELECT DISTINCT 
        l.*,
   ...
   */
  async isElectiveLessonAvailable(lessonId: number, grade: number): Promise<boolean> {
   ...
   */
  async getAvailableElectiveLessonsForClass(classId: number): Promise<Lesson[]> {
    return this.dbManager.getAll(`
      SELECT DISTINCT l.* 
      FROM lessons l
   ...
   */
  async getElectiveLessonsWithStatus(grade: number): Promise<any[]> {
    const currentSchoolType = this.dbManager.getCurrentSchoolType();
    return this.dbManager.getAll(`
      SELECT 
        l.*,
