...
export class CurriculumFixer {
  constructor(private dbManager: DatabaseManager) {}

  async fix8thGradeIssues(): Promise<void> {
  ...
  async fixMathHoursForGrades5to7(): Promise<void> {
  ...
  async fixForeignLanguageHours(): Promise<void> {
    ...
    try {
      ...
      for (const grade of [5, 6, 7, 8]) {
        const expectedHours = gradeHours[grade as keyof typeof gradeHours];
  ...
  async fixTurkishHoursForMiddleSchool(): Promise<void> {
    console.log('\n=== Ortaokul Türkçe Ders Saatleri Düzeltmesi (5-8. Sınıflar) ===');
    try {
      const gradeHours: { [grade: number]: number } = {
        5: 6,
        6: 6,
        7: 5,
        8: 5,
      ...
      for (const grade of [5, 6, 7, 8]) {
        ...
        const turkishLesson = await this.dbManager.getOne(
          `SELECT * FROM lessons WHERE name = 'Türkçe' AND grade = ? AND school_type = 'Ortaokul'`,
          [grade]
        );
        ...
        if (turkishLesson) {
          if (turkishLesson.weekly_hours !== expectedHours) {
            console.log(
              `${grade}. sınıf Türkçe dersi saati güncelleniyor: ${turkishLesson.weekly_hours} -> ${expectedHours}`
            );
            await this.dbManager.runSQL(
              `UPDATE lessons SET weekly_hours = ?, updated_at = datetime('now') WHERE id = ?`,
              [expectedHours, turkishLesson.id]
            );
          } else {
          ...
          await this.dbManager.runSQL(
            `INSERT INTO lessons (name, grade, weekly_hours, is_mandatory, school_type, created_at, updated_at)
             VALUES ('Türkçe', ?, ?, 1, 'Ortaokul', datetime('now'), datetime('now'))`,
            [grade, expectedHours]
          );
        }
      }
  ...
  async addElectiveLessons(): Promise<void> {
...
export async function runCurriculumFix(dbManager: DatabaseManager): Promise<void> {
