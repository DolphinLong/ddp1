...
export class DatabaseManager {
  ...
  private currentSchoolType: string = 'Ortaokul'; // Default to Ortaokul
  // In-memory counters and locks to accelerate highly-concurrent inserts
  private sectionCounters = new Map<string, number>();
  private insertLocks = new Map<string, Promise<unknown>>();
  ...
  private static resolveUserDataPath(): string {
  ...
  private isHighSchoolType(schoolType: string): boolean {
  ...
  async switchDatabase(schoolType: string): Promise<void> {
  ...
  private async createTables(): Promise<void> {
    ...
    const classCols: Array<{name: string}> = await this.db.all("PRAGMA table_info(classes)");
    ...
    const lessonCols: Array<{name: string}> = await this.db.all("PRAGMA table_info(lessons)");
  ...
  private async migrateClassesTable(): Promise<void> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    try {
      ...
      for (const classItem of classes) {
        ...
        await this.db.run(
          'UPDATE classes SET school_type = ? WHERE id = ?',
          [schoolType, classItem.id]
        );
      }
  ...
  private async migrateTeachersTable(): Promise<void> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  private async migrateElectiveTrackingTables(): Promise<void> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  private async seedDefaultData(): Promise<void> {
    ...
    for (const setting of defaultSettings) {
      await this.db.run('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)', 
    ...
    for (const classItem of sampleClasses) {
      await this.db.run('INSERT OR IGNORE INTO classes (school_type, grade, section) VALUES (?, ?, ?)',
        [classItem.school_type, classItem.grade, classItem.section]);
    }

    // Insert sample MEB curriculum lessons based on school type
    let sampleLessons: Array<{name: string, grade: number, weekly_hours: number, is_mandatory: number, school_type: string}> = [];
    if (this.currentSchoolType === 'Ortaokul') {
      ...
      for (let grade = 6; grade <= 8; grade++) {
        const gradeLessons: Array<{name: string, grade: number, weekly_hours: number, is_mandatory: number, school_type: string}> = 
          sampleLessons.filter(lesson => lesson.grade === 5).map(lesson => ({
            ...lesson,
            grade: grade
          }));
      ...
      for (let grade = 10; grade <= 12; grade++) {
        const gradeLessons: Array<{name: string, grade: number, weekly_hours: number, is_mandatory: number, school_type: string}> = 
          sampleLessons.filter(lesson => lesson.grade === 9).map(lesson => ({
            ...lesson,
            grade: grade
          }));
        sampleLessons = sampleLessons.concat(gradeLessons);
      ...
      for (let grade = 2; grade <= 4; grade++) {
        const gradeLessons: Array<{name: string, grade: number, weekly_hours: number, is_mandatory: number, school_type: string}> = 
          sampleLessons.filter(lesson => lesson.grade === 1).map(lesson => ({
            ...lesson,
            grade: grade
          }));
        sampleLessons = sampleLessons.concat(gradeLessons);
    ...
    console.log('Sample lessons by grade:', sampleLessons.reduce((acc: {[key: number]: number}, lesson) => {
      if (!acc[lesson.grade]) acc[lesson.grade] = 0;
      acc[lesson.grade]++;
      return acc;
    }, {}));

    for (const lesson of sampleLessons) {
      await this.db.run('INSERT OR IGNORE INTO lessons (name, grade, weekly_hours, is_mandatory, school_type) VALUES (?, ?, ?, ?, ?)',
        [lesson.name, lesson.grade, lesson.weekly_hours, lesson.is_mandatory, lesson.school_type]);
    }
  ...
  private async initializeElectiveAssignmentStatus(): Promise<void> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    for (const classItem of classes) {
      ...
      const existingStatus = await this.db.get(
        'SELECT id FROM elective_assignment_status WHERE class_id = ?',
        [classItem.id]
      );
  ...
  async runSQL(sql: string, params: any[] = []): Promise<any> {
    ...
    try {
      ...
      if (/^insert\s+into\s+classes\b[\s\S]*\(\s*school_type\s*,\s*grade\s*,\s*section\s*\)/.test(normalized) && params.length === 3) {
        let [schoolType, grade, section] = params as [string, any, string];
        for (let attempt = 0; attempt < 200; attempt++) {
          try {
            const stmt = await this.db.prepare('INSERT INTO classes (school_type, grade, section) VALUES (?, ?, ?)');
        ...
        const runWithLock = async () => {
          ...
          if (counter === undefined) {
            ...
            const rows = await this.getAll(
              'SELECT section FROM classes WHERE school_type = ? AND grade = ? AND section LIKE ?',
              [schoolType, grade, `${baseSection}%`]
            );
            let maxSuffix = -1;
            for (const r of rows) {
              const sec: string = String(r.section ?? '');
              if (sec === baseSection) {
  ...
  async close(): Promise<void> {
    ...
    try { this.cache.clear(); } catch {}
    try { this.sectionCounters.clear(); } catch {}
    try { this.insertLocks.clear(); } catch {}
  ...
  async migrateDataFrom(sourceDbPath: string): Promise<boolean> {
    try {
      ...
      if (this.db) {
        ...
        try {
          ...
          for (const teacher of teachers) {
            await this.db.run(
              'INSERT INTO teachers (id, name, subject, email, phone, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
              [teacher.id, teacher.name, teacher.subject, teacher.email, teacher.phone, teacher.created_at, teacher.updated_at]
            );
          }
          ...
          for (const classItem of classes) {
            await this.db.run(
              'INSERT INTO classes (id, school_type, grade, section, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
              [classItem.id, classItem.school_type, classItem.grade, classItem.section, classItem.created_at, classItem.updated_at]
            );
          }
          ...
          for (const lesson of lessons) {
            await this.db.run(
              'INSERT INTO lessons (id, name, grade, weekly_hours, is_mandatory, school_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
              [lesson.id, lesson.name, lesson.grade, lesson.weekly_hours, lesson.is_mandatory, lesson.school_type, lesson.created_at, lesson.updated_at]
            );
          }
          ...
          for (const scheduleItem of scheduleItems) {
            await this.db.run(
              'INSERT INTO schedule_items (id, class_id, teacher_id, lesson_id, day_of_week, time_slot, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
              [scheduleItem.id, scheduleItem.class_id, scheduleItem.teacher_id, scheduleItem.lesson_id, scheduleItem.day_of_week, scheduleItem.time_slot, scheduleItem.created_at, scheduleItem.updated_at]
            );
          }
          ...
          for (const availability of teacherAvailability) {
            await this.db.run(
              'INSERT INTO teacher_availability (id, teacher_id, day_of_week, time_slot, is_available, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
              [availability.id, availability.teacher_id, availability.day_of_week, availability.time_slot, availability.is_available, availability.created_at, availability.updated_at]
            );
          }
          ...
          for (const counselor of guidanceCounselors) {
            await this.db.run(
              'INSERT INTO guidance_counselors (id, teacher_id, class_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
              [counselor.id, counselor.teacher_id, counselor.class_id, counselor.created_at, counselor.updated_at]
            );
          }
          ...
          for (const assignment of teacherAssignments) {
            await this.db.run(
              'INSERT INTO teacher_assignments (id, teacher_id, lesson_id, class_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
              [assignment.id, assignment.teacher_id, assignment.lesson_id, assignment.class_id, assignment.created_at, assignment.updated_at]
            );
          }
  ...
  async getGuidanceCounselorByClass(classId: number): Promise<any> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  async getGuidanceCounselorByTeacher(teacherId: number): Promise<any[]> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  async getAllGuidanceCounselors(): Promise<any[]> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  async assignGuidanceCounselor(teacherId: number, classId: number): Promise<any> {
    ...
    const classExists = await this.db.get(
    ...
    const result = await this.db.run(
      'INSERT INTO guidance_counselors (teacher_id, class_id) VALUES (?, ?)',
      [teacherId, classId]
    );
  ...
  async getClassAssignedHours(classId: number): Promise<number> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  async assignTeacherToLessonAndClass(teacherId: number, lessonId: number, classId: number): Promise<TeacherAssignment> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }

    const result = await this.db.run(
      'INSERT OR IGNORE INTO teacher_assignments (teacher_id, lesson_id, class_id) VALUES (?, ?, ?)',
      [teacherId, lessonId, classId]
    );

    const assignment = await this.db.get(
      'SELECT * FROM teacher_assignments WHERE teacher_id = ? AND lesson_id = ? AND class_id = ?',
      [teacherId, lessonId, classId]
    );
  ...
  async getTeacherAssignments(teacherId: number): Promise<TeacherAssignment[]> {
    ...
    return await this.db.all(
      `SELECT ta.*, l.name as lesson_name, c.grade, c.section 
       FROM teacher_assignments ta
       JOIN lessons l ON ta.lesson_id = l.id
       JOIN classes c ON ta.class_id = c.id
       WHERE ta.teacher_id = ?
  ...
  async removeTeacherAssignment(teacherId: number, lessonId: number, classId: number): Promise<boolean> {
    ...
    const result = await this.db.run(
      'DELETE FROM teacher_assignments WHERE teacher_id = ? AND lesson_id = ? AND class_id = ?',
      [teacherId, lessonId, classId]
    );
  ...
  async getTeachersForLessonAndClass(lessonId: number, classId: number): Promise<Teacher[]> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    return await this.db.all(
      `SELECT t.* 
       ...
       WHERE ta.lesson_id = ? AND ta.class_id = ?`,
      [lessonId, classId]
    );
  }
  ...
  async getAllClassesForTeacherAssignment(): Promise<any[]> {
  ...
  async getAllOrtaokulLessonsForTeacherAssignment(): Promise<Lesson[]> {
    // Get the current database connection
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    const currentDbLessons: Lesson[] = await this.db.all(`
      SELECT * FROM lessons 
    ...
    for (const dbPath of otherDbPaths) {
      if (fs.existsSync(dbPath)) {
        try {
          ...
          const lessons: Lesson[] = await otherDb.all(`
            SELECT * FROM lessons 
            WHERE grade >= 5 AND grade <= 8 
            ORDER BY grade ASC, name ASC
          `);
  ...
  async removeDuplicateClasses(): Promise<number> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    for (const dup of duplicates) {
      const result = await this.db.run(
        'DELETE FROM classes WHERE school_type = ? AND grade = ? AND section = ? AND id != ?',
        [dup.school_type, dup.grade, dup.section, dup.keep_id]
      );
  ...
  async removeDuplicateLessons(): Promise<number> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    for (const dup of duplicates) {
      const result = await this.db.run(
        'DELETE FROM lessons WHERE name = ? AND grade = ? AND school_type = ? AND id != ?',
        [dup.name, dup.grade, dup.school_type, dup.keep_id]
      );
      removedCount += result.changes || 0;
    }
  ...
  async removeDuplicateTeachers(): Promise<number> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    ...
    for (const dup of duplicates) {
      const result = await this.db.run(
        'DELETE FROM teachers WHERE name = ? AND subject = ? AND id != ?',
        [dup.name, dup.subject, dup.keep_id]
      );
      removedCount += result.changes || 0;
    }
  ...
  async cleanupDatabase(): Promise<{ classes: number; lessons: number; teachers: number }> {
   ...
   */
  async getCached<T>(key: string, queryFn: () => Promise<T>, ttl: number = this.DEFAULT_CACHE_TTL): Promise<T> {
   ...
   */
  async getCachedElectiveStatuses(): Promise<any[]> {
  ...
  async getCachedActiveAlerts(): Promise<any[]> {
  ...
  async getCachedElectiveDistribution(): Promise<any[]> {
